...
//   (一大群一起上船,同 glean「多算給你」語意),點一下整排整列＋旁邊一圈歸艙。
...
(function () {
  ...
  const T = {
    ...
    how: '動物們成群來到方舟前!點一隻、再點旁邊的一隻交換位置;排成一排 3 隻同款就「一起上船」,新的動物會從遠方走來補上。連出 4 隻以上會出現彩虹方塊——點一下,整排整列＋旁邊一圈一起上船!湊滿目標對數,大家都平安。放心慢慢來——沒有步數限制。',
    pick: '天要下雨了,挪亞在船邊等你:',
    ...
    crowGo: '烏鴉飛來飛去,飛走了',
    rainbowBorn: '彩虹!點它,整排整列＋旁邊一圈一起上船',
    rainbowGo: '一大群一起上船了!',
  ...
  class Game {
    constructor(canvas) {
      ...
      this._bgmCfg = { vol: 0.9, type: 'triangle', step: 0.34, scale: [261.63,293.66,329.63,392,440,523.25,587.33,659.25], bass: [130.81,196], bassEvery: 8, btnBg: 'rgba(44,60,28,0.5)', btnFg: '#f4f8e8' }
    ...
    _draw() {
      const { ctx, W, H } = this
      if (!W) return
      // 雨後的天空+青草地(07-24 使用者點名:再淡一點+常駐彩虹=立約記號,創 9:13)
      const sky = ctx.createLinearGradient(0, 0, 0, H)
      sky.addColorStop(0, '#c4d8ec'); sky.addColorStop(0.55, '#e0eaf2'); sky.addColorStop(0.72, '#bcd8a4'); sky.addColorStop(1, '#a2c886')
    ...
    // 🌈 常駐背景彩虹(創 9:13 立約記號;柔和粉彩,掛在天空不搶戲)
    _bgRainbow() {
      const { ctx } = this
      const COLORS = ['#f0b4b4', '#f0d2a6', '#f0eab0', '#c0e0b2', '#b2d2e8', '#c8bce8']
      ctx.save()
      ...
      COLORS.forEach((col, i) => {
        ctx.strokeStyle = col
        ctx.beginPath()
        ctx.arc(852, 460, 330 - i * 12, Math.PI * 1.05, Math.PI * 1.95)
        ctx.stroke()
      })
      ctx.restore()
    }
    ...
    // 💧 方舟停在水邊(洪水漸退;底部水面+波光+蘆葦,07-24 使用者點名)
    _waterside() {
      const { ctx } = this
      ctx.save()
    ...
    // 🌦 太陽雨(07-24):雨絲+常駐彩虹=雨中有虹(創 9:13 立約的天空);reduced-motion 不下
    _rain() {
      if (this.reduced) return
      const { ctx } = this
      ctx.strokeStyle = 'rgba(150,190,230,0.32)'
      ctx.lineWidth = 1.5
      ctx.beginPath()
      for (let i = 0; i < 36; i++) {
        const speed = 300 + (i % 5) * 45
        const x0 = (i * 167.7) % VW
        const y = ((i * 91 + this._t * speed) % (VH + 60)) - 30
        const x = x0 - y * 0.08
        ctx.moveTo(x, y); ctx.lineTo(x + 1.4, y + 13)
      }
      ctx.stroke()
    }
